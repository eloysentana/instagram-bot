...
class Bot(Browser):
    ...
    def get_user_connections_from_web(self, limit: Optional[int] = None, followers: bool = True, force_search: bool = False):
        ...
        #  or user has less connections than records (otherwise enable option -> Force Search)
        ...
        while perf_counter() - timestamp < self.timeout:  # Timer to prevent being in a loop if someone unfollows while searching
        ...
        for comment in comments.generate():
            ...
            while not success:
                ...
                if not has_input:
                    ...
                    except WebDriverException:  # This would be pretty sure a char/emoji not in BMP because ChromeDriver doesn't support.
